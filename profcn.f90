...
!
    use param
!
!  Developed originally in about 2000 by arnie lee van buren and jeffrey
!  boisvert. Updated several times since then. For more information see:
!  GitHub.com/MathieuandSpheroidalWaveFunctions/Prolate_swf. Especially see
!  the readme file, example input and output files and two relevant journal
!  articles.
!
!  purpose:     To calculate the first and second kind prolate
!               radial functions r1 and r2 and their first
!               derivatives r1d and r2d for a range of orders m,
...
!  assumed to be in the directory of profcn.for. Profcn.dat
...
!  values are set below to 8 and 16, respectively. They should be changed
!  to the kind values for double precision and quadruple precision if those
!  values are different than these.
!
5       kindd=8
        kindq=16
!
...
!  changed if desired. See comments below about changing minacc
!
...
200           format(15x,'sub. error in forming wronskian = ',i3, &
              ...
              if(naccint.ge.minacc.and.ndec-jsub.le.naccint.and. &
                      iopleg.ne.0) iopleg=0
              if(naccint.lt.minacc.and.x1.le.0.1e0_knd.and. &
                 iopleg.eq.0.and.l.ge.legstart.and. &
                 jsub.le.ndec-naccint.and.jsub.le.ndec-naccrp) iopleg=1
...
320           continue
              ...
              if(iopint.ne.0.and.naccleg.gt.max(naccint,naccintp) &
                 .and.nacclegp.gt.max(naccint,naccintp) &
                 .and.jflagl.eq.0) iopint=0
              nacclegp=naccleg
if (debug) then
...
630           format(15x,'accuracy in decimal digits = ',i2,5x, &
              ...
              if((naccd.ge.naccdp.or.naccd.ge.minacc.or.naccd.ge.naccrp &
                  ...
                  nee.ne.neta.and.insflag.eq.0) iopnee=0
              naccdp=naccd
...
710           format(1x,i6,2x,2(f17.14,1x,i6,2x))
              ...
              if(iopint.ne.0) naccintp=naccint
                ...
                if(ndec-jsub.gt.naccr.and.x1.le.0.4e0_knd.and. &
                   iopleg.eq.0.and.l.ge.legstart) iopleg=1
                ...
                if(iopeta.eq.0.and.x1.ge.0.00065e0_knd.and.iopneu.eq.0 &
                   .and.iopleg.ne.2) iopeta=1
                end if
              if(iopeta.eq.4) iopeta=1
              if((naccr.lt.legtest.or.naccrp.lt.legtest).and. &
                  x1.le.0.01e0_knd.and.l.gt.legstart.and.iopleg.eq.0) &
                  iopleg=1
              if(iopneu.eq.0.and.naccr.lt.minacc.and.naccrp.lt.minacc &
                 .and.ndec-jsub.gt.naccr.and.ndec-jsub.gt.naccrp.and. &
                 x1.ge.0.00065e0_knd) iopneu=1
              naccrp=naccr
...
if (debug) then
                ...
                if(knd.eq.kindd.and.iopang.eq.2) write(50,780) s1c(jarg),is1e(jarg),s1dc(jarg),is1de(jarg)
                if(knd.eq.kindq.and.iopang.eq.1) write(50,790) s1c(jarg),is1e(jarg)
...
180     continue
        ...
        if(ndec-naccns.ge.naccr+2) iopnee=0
