...
!  set array dimensions
    ...
    if(x1 >= 0.00065e0_knd) maxn = 2 * (lnump * (-18.5e0_knd- &
                  20.e0_knd * log10(x1))+ &
                  5 * ndec + 4 * maxm + c + 05000) + maxm + 5
    if(x1 > 0.08e0_knd) maxn = 2 * (lnump * (0.5e0_knd- &
                 3.0e0_knd * log10(x1))+ &
...
!  determine first kind prolate angular function
       ...
       call s1leg(l, m, c, iopang, iopnorm, barg, narg, lims1, ndec, maxt, &
        ...
        do 810 jarg = 1, narg
if (debug) then
        if(knd == kindd .and. ioparg == 0) write(50, 730) arg(jarg), naccs(jarg)
        if(knd == kindq .and. ioparg == 0) write(50, 735) arg(jarg), naccs(jarg)
730       format(1x,'theta = ',f19.14,' accuracy = ',i5,' digits.')
735       format(1x,'theta = ',f35.30,' accuracy = ',i5,' digits.')
        if(knd == kindd .and. ioparg == 1) write(50, 740) barg(jarg), naccs(jarg)
        if(knd == kindq .and. ioparg == 1) write(50, 745) barg(jarg), naccs(jarg)
740       format(1x,'eta = ',f17.14,'  accuracy = ',i5, ' digits.')
745       format(1x,'eta = ',f33.30,'  accuracy = ',i5, ' digits.')
end if
if (output) then
        if(ioparg == 0 .and. iopang == 1) write(30, 750) arg(jarg), s1c(jarg), is1e(jarg), naccs(jarg)
        if(ioparg == 0 .and. iopang == 2) write(30, 760) arg(jarg), s1c(jarg), is1e(jarg), s1dc(jarg), is1de(jarg), naccs(jarg)
        if(ioparg == 1 .and. iopang == 1) write(30, 750) barg(jarg), s1c(jarg), is1e(jarg), naccs(jarg)
        if(ioparg == 1 .and. iopang == 2) write(30, 760) barg(jarg), s1c(jarg), is1e(jarg), s1dc(jarg), is1de(jarg), naccs(jarg)
